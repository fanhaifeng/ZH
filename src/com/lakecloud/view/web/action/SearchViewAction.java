...
public class SearchViewAction {
	...
	public ModelAndView search(HttpServletRequest request,
		...
		pageSize = pageSize == null ? Globals.DEFAULT_PAGE_SIZE : pageSize;// 设置搜索页数量
		if (type.equals("store")) {
			mv = new JModelAndView("store_list.html", configService
			...
			if (storepoint != null && !storepoint.equals("")) {
				sqo
						.addQuery("obj.sp.store_evaluate1", new SysMap(
								"sp_store_evaluate1",
			...
			if (area_name != null && !area_name.equals("")) {
				...
				sqo
						.addQuery("obj.area.parent.parent.areaName",
								new SysMap("areaName", "%" + area_name.trim()
										+ "%"), "like", "or");
			}
			sqo
					.addQuery("obj.store_status",
							new SysMap("store_status", 2), "=");
			sqo.setPageSize(20);
			...
			if(CommUtil.null2String(orderBy).equals("new_product")){//新品
				Calendar now=Calendar.getInstance();
				now.add(Calendar.MONTH,-1);//一个月之前
			...
			if (!CommUtil.null2String(order_by).equals("")) {
				if (LuceneVo.GOODS_COLLECT.equals(order_by)
						|| LuceneVo.GOODS_SALENUM.equals(order_by)) {
			...
			LuceneResult pList = lucene.search(keyword, CommUtil
					.null2Int(currentPage), CommUtil.null2Int(pageSize),
					CommUtil.null2Double(store_price_begin), CommUtil
							.null2Double(store_price_end), null, sort, null,
					null, null, null,querys);
			/*
			 * for (LuceneVo vo : pList.getVo_list()) { Goods goods =
			 * this.goodsService.getObjById(vo.getVo_id());
