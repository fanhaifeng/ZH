...
import com.lakecloud.foundation.service.impl.GoodsFloorGoodsServiceImpl;
...
public class GoodsFloorManageAction {
	...
	public String goods_floor_del(HttpServletRequest request,
			HttpServletResponse response, String mulitId, String currentPage) throws UnsupportedEncodingException {
		...
		for (String id : ids) {
			if (!id.equals("")) {
				GoodsFloor goodsfloor = this.goodsfloorService.getObjById(Long
						.parseLong(id));
				//判断该楼层是否可删除
				if(goodsfloor.getChilds().size()>0){
					flag=1;
					requestString +=goodsfloor.getGf_name()+"有下级楼层，不能删除。\\n";	
				}else{
					if(goodsfloor.getGf_gc_goods()!=null && !goodsfloor.getGf_gc_goods().isEmpty() && !goodsfloor.getGf_gc_goods().equals("{}")){
						flag=1;
						requestString +=goodsfloor.getGf_name()+"下有商品，不能删除。\\n";	
					}else{
		...
		return "redirect:goods_floor_list.htm?currentPage=" + currentPage+"&message="+ URLEncoder.encode(requestString, "UTF-8")+"&flag="+flag;
	}
	...
	@SecurityMapping(title = "楼层分类下级加载", value = "/admin/goods_floor_data.htm*", rtype = "admin", rname = "首页楼层", rcode = "goods_floor", rgroup = "商品")
	...
	public ModelAndView goods_floor_template(HttpServletRequest request,
		...
		ModelAndView mv = new JModelAndView(
				"admin/blue/goods_floor_template.html", configService
						.getSysConfig(),
	...
	public ModelAndView goods_floor_class(HttpServletRequest request,
			HttpServletResponse response, String currentPage, String id) {
		ModelAndView mv = new JModelAndView(
				"admin/blue/goods_floor_class.html", configService
						.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		GoodsFloor obj = this.goodsfloorService.getObjById(CommUtil
				.null2Long(id));
	...
	public ModelAndView goods_floor_class_load(HttpServletRequest request,
		...
		ModelAndView mv = new JModelAndView(
				"admin/blue/goods_floor_class_load.html", configService
						.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
	...
	public ModelAndView goods_floor_gc_goods(HttpServletRequest request,
			HttpServletResponse response, String currentPage, String id) {
		ModelAndView mv = new JModelAndView(
				"admin/blue/goods_floor_gc_goods.html", configService
						.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		GoodsFloor obj = this.goodsfloorService.getObjById(CommUtil
				.null2Long(id));
	...
	public String goods_floor_gc_goods_save(HttpServletRequest request,
		...
		for (int i = 0; i < id_list.length; i++) {
			if (!id_list[i].equals("")) {
				...
				Goods goods = this.goodsService.getObjById(Long
						.valueOf(id_list[i]));
				save_goods_floor_goods(obj, goods);
			}
		}
	...
	public ModelAndView goods_floor_list_goods(HttpServletRequest request,
			HttpServletResponse response, String currentPage, String id) {
		ModelAndView mv = new JModelAndView(
				"admin/blue/goods_floor_list_goods.html", configService
						.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		GoodsFloor obj = this.goodsfloorService.getObjById(CommUtil
				.null2Long(id));
	...
	public ModelAndView goods_floor_left_adv(HttpServletRequest request,
		...
		ModelAndView mv = new JModelAndView(
				"admin/blue/goods_floor_left_adv.html", configService
						.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		GoodsFloor obj = this.goodsfloorService.getObjById(CommUtil
				.null2Long(id));
	...
	public ModelAndView goods_floor_right_adv(HttpServletRequest request,
			HttpServletResponse response, String id) {
		ModelAndView mv = new JModelAndView(
				"admin/blue/goods_floor_right_adv.html", configService
						.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		GoodsFloor obj = this.goodsfloorService.getObjById(CommUtil
				.null2Long(id));
	...
	public ModelAndView goods_floor_brand(HttpServletRequest request,
			HttpServletResponse response, String id) {
		ModelAndView mv = new JModelAndView(
				"admin/blue/goods_floor_brand.html", configService
						.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		GoodsFloor obj = this.goodsfloorService.getObjById(CommUtil
				.null2Long(id));
	...
	public ModelAndView goods_floor_brand_load(HttpServletRequest request,
		...
		ModelAndView mv = new JModelAndView(
				"admin/blue/goods_floor_brand_load.html", configService
						.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		...
		IPageList pList = this.goodsBrandService.list(qo);
		CommUtil.saveIPageList2ModelAndView(CommUtil.getURL(request)
				+ "/admin/goods_floor_brand_load.htm", "", "&name="
	...
	public ModelAndView goods_floor_list_goods_load(HttpServletRequest request,
			...
			String goods_name) {
		ModelAndView mv = new JModelAndView(
				"admin/blue/goods_floor_list_goods_load.html", configService
						.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
	...
	private boolean save_goods_floor_goods(GoodsFloor obj, Goods goods) {
		GoodsFloorGoods goodsFloorGoods = new GoodsFloorGoods();
		goodsFloorGoods.setGoods(goods);
		goodsFloorGoods.setGoodsFloor(obj.getParent());
		goodsFloorGoods.setGoodsFloor2(obj);
		goodsFloorGoods.setGoods_store(goods.getGoods_store());
		goodsFloorGoods.setAddTime(new Date());
		return this.goodsFloorGoodsService.save(goodsFloorGoods);
	}

	private boolean delete_goods_floor_goods(GoodsFloor obj) {
		boolean result = true;
		Map params = new HashMap();
		params.put("goods_floor", obj);
		List<GoodsFloorGoods> list = this.goodsFloorGoodsService
				.query(
						"select obj from GoodsFloorGoods obj where obj.goodsFloor2=:goods_floor",
						params, -1, -1);
		for (GoodsFloorGoods goodsFloorGoods : list) {
			this.goodsFloorGoodsService.delete(goodsFloorGoods.getId());
		}
		return result;
	}
}