...
//			}
	...
	public ModelAndView user_save(HttpServletRequest request,
			...
			String list_url, String add_url, String password,String area) {
		WebForm wf = new WebForm();
		User user = null;
		if (id.equals("")) {
		...
		if(area != null && !area.equals("")){		
			Area area1 = this.areaService.getObjById(CommUtil.null2Long(area));
			if(area1!=null){
				user.setArea(area1);
				if(area1.getBranch()!=null){
					Branch branch = this.branchService.getObjById(area1.getBranch().getId());
					if(branch !=null){
