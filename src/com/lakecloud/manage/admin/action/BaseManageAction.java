...
public class BaseManageAction {
	...
	public void login_success(HttpServletRequest request,
		...
		if(user.getStore()!=null){
			session.setAttribute("store_id", user.getStore().getId());
		}
		...
		user.setLogin_role(login_role);
		user.setLoginCount(user.getLoginCount() + 1);
		this.userService.update(user);
		
		session.setAttribute("last_login_role", login_role);// 设置登录标记
		//设置浏览器cookie
		Cookie cookie = new Cookie("lastLogin_role",login_role);
		...
		if (ajax_login) {
			...
			}else if(login_role.equals("seller")){
				if(role.indexOf("SELLER") >= 0){
	...
	public void logout_success(HttpServletRequest request,
		...
		HttpSession session = request.getSession(false);
		String login_role;
		if(session.getAttribute("login_role") == null){
			login_role = "";
		}else{
			login_role = session.getAttribute("login_role").toString();
		}
		//根据登录角色进行退出登录
		String targetUrl = CommUtil.getURL(request) + "/user/login.htm";
		if("admin".equals(login_role)){
			targetUrl = CommUtil.getURL(request) + "/admin/login.htm";
		}else if("seller".equals(login_role)){
		...
		if (iskyshop_view_type != null && !iskyshop_view_type.equals("")) {
			if (iskyshop_view_type.equals("weixin")) {
				targetUrl=CommUtil.getURL(request) + "/weixin/login.htm";
	...
	public ModelAndView login_error(HttpServletRequest request,
		...
		if (iskyshop_view_type != null && !iskyshop_view_type.equals("")) {
			...
			if (login_role.equals("admin")) {
				...
				mv = new JModelAndView("login_error.html",
						configService.getSysConfig(),
						this.userConfigService.getUserConfig(), 1, request,
						response);
			} else {
	 ...
	 * @return
	 */
	@SecurityMapping(title = "websiteCss设置", value = "/admin/set_websiteCss.htm*", rtype = "admin", rname = "平台样式设置", rcode = "admin_set_websiteCss", rgroup = "设置")
	...
	public void weixin_login(HttpServletRequest request,
			HttpServletResponse response) throws IOException, ServletException {
		String username=request.getParameter("username")==null?"":request.getParameter("username");
		String sequence=request.getParameter("sequence")==null?"":request.getParameter("sequence");
		String mark=request.getParameter("mark")==null?"":request.getParameter("mark");
		String url=request.getParameter("url")==null?"":request.getParameter("url");
		HttpSession session = request.getSession();
		if(!url.equals("")&&mark.equals("qrcode")){
			session.setAttribute("loginType", "qr_code");
			session.setAttribute("resultUrl", url);
		}else{
		...
		if(sequence.length()<25)
			...
			LoginFlag lf=new LoginFlag();
			sequence=lf.decodeLogin(sequence);		
		}
		response.sendRedirect(CommUtil.getURL(request) + "/iskyshop_weixin_login.htm?username="+username+"&password="+sequence);
	}
	/**
	 * APP登录
	 * @param request
	 ...
	 * @throws IOException
	 */
	@Log(title = "APP用户登陆", type = LogType.LOGIN)
	@RequestMapping("/weixin_app_login.htm")
	public void weixin_app_login(HttpServletRequest request,
			HttpServletResponse response) throws IOException {
		User user=(User) request.getSession().getAttribute("user");
		...
		if(login_type!=null&&resultUrl!=null&&login_type.equals("qr_code")&&!resultUrl.equals("")){
			response.sendRedirect(CommUtil.getURL(request) + resultUrl);
		}else{
			response.sendRedirect(CommUtil.getURL(request) + "/weixin/platform/index.htm");			
		}
	}
	@RequestMapping("/app_login_error.htm")
	public void app_login_error(HttpServletRequest request,
			HttpServletResponse response) throws IOException {
