...
public class StatManageAction {
	...
	private void execute() throws Exception {
		...
		for (Store store : stores) {
			...
			//商品评价统计Map<商品id,[商品评价总分,商品评价次数]>
			Map<Long,BigDecimal[]> map=new HashMap<Long, BigDecimal[]>();
			//存放是需要更新的商品
			Map<Long,Goods> goodsList=new HashMap<Long, Goods>();
			for (Evaluate eva1 : evas) {
					//统计商品描述相符评分
					Goods goods=eva1.getEvaluate_goods();
					Long id = goods.getId();
					if(map.containsKey(id)){
						BigDecimal[] value = map.get(id);
						value[0]=value[0].add(eva1.getDescription_evaluate());
						value[1]=value[1].add(new BigDecimal(1));
					}else{
						BigDecimal[] value=new BigDecimal[2];
						value[0]=eva1.getDescription_evaluate();
						value[1]=new BigDecimal(1);
						map.put(id, value);
						goodsList.put(id, goods);
					}
				//统计商品描述相符评分结束
			...
			for (Long key : map.keySet()) {
				...
				BigDecimal evaluate = value[0].divide(value[1]);
				Goods goods=goodsList.get(key);
				goods.setDescription_evaluate(evaluate);
...
//		}
		/*
		//赊销短信
		String queryString = "select obj from OrderForm obj where obj.charge_Num>0 and obj.charge_Day>0";
		List<OrderForm> orderFormList = this.orderFormService.query(queryString,null,-1,-1);
		Calendar calendar = Calendar.getInstance();
		for(OrderForm orderForm:orderFormList){
			calendar.setTime(new Date());
			if(orderForm.getExpireTime()!=null){
				if(orderForm.getExpireTime().before(calendar.getTime()) && orderForm.getAdvance_notice()==0){
					//过期提示发送短信提醒经销商及用户，有一笔赊销已经过期
					System.out.println("赊销过期");
					orderForm.setAdvance_notice(1);
					this.orderFormService.update(orderForm);
					
					this.send_charge_sms(orderForm,orderForm.getStore().getUser().getTelephone(),"sms_to_seller_delay_charge_notify");				
					this.send_charge_sms(orderForm,orderForm.getUser().getTelephone(), "sms_to_buyer_delay_notify");
				}else{
					calendar.add(Calendar.DATE,15);	
					SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");
					System.out.println(format.format(calendar.getTime()) + "赊销到期："+format.format(orderForm.getExpireTime()));
					if(orderForm.getExpireTime().before(calendar.getTime()) && orderForm.getDelay_notice()==0){	
						//提前15天通知订单过期
						System.out.println("提前15天通知");
						orderForm.setDelay_notice(1);
						this.orderFormService.update(orderForm);
						this.send_charge_sms(orderForm,orderForm.getStore().getUser().getTelephone(), "sms_to_seller_advance_charge_notify");				
						this.send_charge_sms(orderForm,orderForm.getUser().getTelephone(), "sms_to_buyer_advance_notify");
					}	
				}	
			}
		}*/
...
//		}
	...
	private void send_charge_sms(OrderForm order,
