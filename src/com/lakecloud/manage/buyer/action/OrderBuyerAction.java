...
public class OrderBuyerAction {
	...
	public ModelAndView order(HttpServletRequest request,
		...
		if (!CommUtil.null2String(order_status).equals("")) {
			...
			if (order_status.equals("order_needpay")) {// 货到付款待发货
				ofqo.addQuery("obj.order_status",
...
//			
//			//取消订单后如果有赊销金额要将赊销金额返还
//			if(obj.getCharge_Num().compareTo(BigDecimal.ZERO) >= 0 ){	
//				User user = this.userService.getObjById(SecurityUserHolder
//						.getCurrentUser().getId());
//				//获取该用户的赊销信息
//				String querString ="select obj from Charge obj where obj.store.id=:store_id and obj.user.id=:user_id";
//				Map<String,Long> paMap = new HashMap<String, Long>();
//				paMap.put("store_id",user.getStore().getId());
//				paMap.put("user_id",user.getId());
//				
//				List<Charge>  chargeList = this.chargeService.query(querString, paMap, -1, -1);
//				if(chargeList!=null && chargeList.size()>0){
//					Charge charge = chargeList.get(0);
//					charge.setUsedPayNum(charge.getUsedPayNum().subtract(obj.getCharge_Num()));
//					charge.setPaymentNum(charge.getPaymentNum()+obj.getCharge_Num());
...
//		return "redirect:order.htm?currentPage=" + currentPage;
	...
	public String order_cofirm_save(HttpServletRequest request,
			HttpServletResponse response, String id, String currentPage)
			throws Exception {
		return this.orderFormService.order_cofirm_save(request, response,id,currentPage);
	}
	...
	public String order_return_apply_save(HttpServletRequest request,
		...
		if (obj.getUser().getId()
			...
			if(ret){
				OrderFormLog ofl = new OrderFormLog();
				ofl.setAddTime(new Date());
				ofl.setLog_info("申请退货");
				ofl.setLog_user(SecurityUserHolder.getCurrentUser());
				ofl.setOf(obj);
				ofl.setState_info(obj.getReturn_content());
				this.orderFormLogService.save(ofl);
			}
//			if (this.configService.getSysConfig().isEmailEnable()) {
//				this.send_email(request, obj,
